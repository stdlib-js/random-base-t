...
function factory() {
	...
	if ( arguments.length === 0 ) {
		...
		if ( isObject( arguments[ 0 ] ) ) {
			...
			if ( hasOwnProp( opts, 'copy' ) && !isBoolean( opts.copy ) ) {
				throw new TypeError( format( '0p72o,GE', 'copy', opts.copy ) );
			}
			if ( hasOwnProp( opts, 'prng' ) ) {
				if ( !isFunction( opts.prng ) ) {
					throw new TypeError( format( '0p76u,JI', 'prng', opts.prng ) );
				}
				rnorm = randn({
					'prng': opts.prng
				});
				...
				if ( hasOwnProp( opts, 'state' ) && !isUint32Array( opts.state ) ) {
					throw new TypeError( format( '0p76z,JJ', 'state', opts.state ) );
				}
				opts = assign( {}, opts );
				if ( opts.copy === false ) {
					FLG = false;
			...
			v = arguments[ 0 ];
			if ( !isPositive( v ) ) {
				throw new TypeError( format( '0p77a,NV', v ) );
			}
			opts = {
				'copy': false
			};
		...
		v = arguments[ 0 ];
		if ( !isPositive( v ) ) {
			throw new TypeError( format( '0p771,NQ', v ) );
		}
		opts = arguments[ 1 ];
		if ( !isObject( opts ) ) {
			throw new TypeError( format( '0p72V,FD', opts ) );
		}
		if ( hasOwnProp( opts, 'copy' ) && !isBoolean( opts.copy ) ) {
			throw new TypeError( format( '0p72o,GE', 'copy', opts.copy ) );
		}
	...
	*/
	function setState( s ) {
		if ( !isUint32Array( s ) ) {
			throw new TypeError( format( '0p770,NW', s ) );
		}
		if ( FLG ) {
